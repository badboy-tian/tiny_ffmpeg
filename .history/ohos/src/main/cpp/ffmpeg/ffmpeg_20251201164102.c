...
static int frame_data_ensure(AVBufferRef **dst, int writable) {
  ...
  if (!src || (writable && !av_buffer_is_writable(src))) {
    ...
    *dst =
        av_buffer_create((uint8_t *)fd, sizeof(*fd), frame_data_free, NULL, 0);
    if (!*dst) {
      av_buffer_unref(&src);
      av_freep(&fd);
      return AVERROR(ENOMEM);
...
int check_avoptions_used(const AVDictionary *opts,
                         const AVDictionary *opts_used, void *logctx,
                         int decode) {
  const AVClass *class = avcodec_get_class();
  const AVClass *fclass = avformat_get_class();

  const int flag =
      decode ? AV_OPT_FLAG_DECODING_PARAM : AV_OPT_FLAG_ENCODING_PARAM;
...
static void print_report(int is_last_report, int64_t timer_start,
  ...
  for (OutputStream *ost = ost_iter(NULL); ost; ost = ost_iter(ost)) {
    const float q =
        ost->enc ? atomic_load(&ost->quality) / (float)FF_QP2LAMBDA : -1;
  ...
  if (total_size < 0)
    ...
    av_bprintf(&buf, "size=%8.0fKiB time=", total_size / 1024.0);
...
static void print_stream_maps(void) {
  ...
  for (OutputStream *ost = ost_iter(NULL); ost; ost = ost_iter(ost)) {
    ...
    av_log(NULL, AV_LOG_INFO, "  Stream #%d:%d -> #%d:%d",
           ost->ist->file->index, ost->ist->index, ost->file->index,
           ost->index);
    if (ost->enc) {
...
// Global variables for session management and error handling
jmp_buf ex_buf__;
int64_t current_ffmpeg_session_id = 0;
...
// Override exit_program to use longjmp instead of exit()
...
int exe_ffmpeg_cmd_with_session(int64_t sessionId, int argc, char **argv) {
  ...
  ret = received_nb_signals                   ? 255
        : (ret == FFMPEG_ERROR_RATE_EXCEEDED) ? 69
                                              : ret;
