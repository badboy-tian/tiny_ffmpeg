...
void init_opts(void) { av_dict_set(&sws_dict, "flags", "bicubic", 0); }
...
void log_callback_help(void *ptr, int level, const char *fmt, va_list vl) { vfprintf(stdout, fmt, vl); }
...
int64_t parse_time_or_die(const char *context, const char *timestr, int is_duration) {
    ...
    if (av_parse_time(&us, timestr, is_duration) < 0) {
        av_log(NULL, AV_LOG_FATAL, "Invalid %s specification for %s: %s\n", is_duration ? "duration" : "date", context,
               timestr);
        exit_program(1);
    }
    return us;
...
#else
static inline void prepare_app_arguments(int *argc_ptr, char ***argv_ptr) { /* nothing to do */ }
...
int opt_default(void *optctx, const char *opt, const char *arg) {
    ...
    if (!consumed && (o = opt_find(&sc, opt, NULL, 0, AV_OPT_SEARCH_CHILDREN | AV_OPT_SEARCH_FAKE_OBJ))) {
        ...
        sws_freeContext(sws);
        if (!strcmp(opt, "srcw") || !strcmp(opt, "srch") || !strcmp(opt, "dstw") || !strcmp(opt, "dsth") ||
            !strcmp(opt, "src_format") || !strcmp(opt, "dst_format")) {
            av_log(NULL, AV_LOG_ERROR,
                   "Directly using swscale dimensions/format options is not supported, please use the -s or -pix_fmt "
                   "options\n");
            return AVERROR(EINVAL);
        }
        if (ret < 0) {
    ...
    while ((c = *(template ++))) {
        if (c == '%') {
            ...
            case 'p':
                ...
                break;
            case 't':
                av_bprintf(bp, "%04d%02d%02d-%02d%02d%02d", tm->tm_year + 1900, tm->tm_mon + 1, tm->tm_mday,
...
static int init_report(const char *env) {
    ...
    av_log(NULL, AV_LOG_INFO,
           ...
           "Report written to \"%s\"\n",
           program_name, tm->tm_year + 1900, tm->tm_mon + 1, tm->tm_mday, tm->tm_hour, tm->tm_min, tm->tm_sec,
           filename.str);
    av_bprint_finalize(&filename, NULL);
    return 0;
}

int opt_report(const char *opt) { return init_report(NULL); }
...
#if CONFIG_NONFREE
    printf("This version of %s has nonfree parts compiled in.\n"
    ...
    while ((filter = av_filter_iterate(&opaque))) {
        ...
        *descr_cur = 0;
        printf(" %c%c%c %-17s %-10s %s\n", filter->flags & AVFILTER_FLAG_SUPPORT_TIMELINE ? 'T' : '.',
               filter->flags & AVFILTER_FLAG_SLICE_THREADS ? 'S' : '.', filter->process_command ? 'C' : '.',
...
static void show_help_filter(const char *name) {
    ...
    if (f->priv_class)
        show_help_children(f->priv_class,
                           AV_OPT_FLAG_VIDEO_PARAM | AV_OPT_FLAG_FILTERING_PARAM | AV_OPT_FLAG_AUDIO_PARAM);
...
FILE *get_preset_file(char *filename, size_t filename_size, const char *preset_name, int is_path,
    ...
    if (is_path) {
        ...
        for (i = 0; i < 3 && !f; i++) {
            ...
            f = fopen(filename, "r");
            if (!f && codec_name) {
                snprintf(filename, filename_size, "%s%s/%s-%s.ffpreset", base[i], i != 1 ? "" : "/.ffmpeg", codec_name,
                         preset_name);
                f = fopen(filename, "r");
            }
        }
...
AVDictionary *filter_codec_opts(AVDictionary *opts, enum AVCodecID codec_id, AVFormatContext *s, AVStream *st,
                                AVCodec *codec) {
