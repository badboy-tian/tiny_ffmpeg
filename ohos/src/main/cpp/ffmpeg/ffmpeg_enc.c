...
static EncoderPriv *ep_from_enc(Encoder *enc) { return (EncoderPriv *)enc; }
...
int enc_alloc(Encoder **penc, const AVCodec *codec, Scheduler *sch,
              unsigned sch_idx, void *log_parent) {
  EncoderPriv *ep;
  int ret = 0;
...
static int hw_device_setup_for_encode(Encoder *e, AVCodecContext *enc_ctx,
  ...
  for (int i = 0;; i++) {
    ...
    if (frames_ref &&
      ...
      av_log(e, AV_LOG_VERBOSE,
             "Using input "
             "frames context (format %s) with %s encoder.\n",
             av_get_pix_fmt_name(enc_ctx->pix_fmt), enc_ctx->codec->name);
...
int enc_open(void *opaque, const AVFrame *frame) {
  ...
  case AVMEDIA_TYPE_AUDIO:
    av_assert0(frame->format != AV_SAMPLE_FMT_NONE && frame->sample_rate > 0 &&
  ...
  case AVMEDIA_TYPE_VIDEO: {
    av_assert0(frame->format != AV_PIX_FMT_NONE && frame->width > 0 &&
               frame->height > 0);
  ...
  ret = hw_device_setup_for_encode(e, enc_ctx,
                                   frame ? frame->hw_frames_ctx : NULL);
  if (ret < 0) {
    av_log(e, AV_LOG_ERROR, "Encoding hardware device setup failed: %s\n",
           av_err2str(ret));
    return ret;
  }
...
static int do_subtitle_out(OutputFile *of, OutputStream *ost,
                           const AVSubtitle *sub, AVPacket *pkt) {
  Encoder *e = ost->enc;
  EncoderPriv *ep = ep_from_enc(e);
...
void enc_stats_write(OutputStream *ost, EncStats *es, const AVFrame *frame,
  ...
  for (size_t i = 0; i < es->nb_components; i++) {
    ...
    case ENC_STATS_FILE_IDX:
      avio_printf(io, "%d", ost->file->index);
      continue;
    ...
    if (frame) {
      ...
      case ENC_STATS_SAMPLE_NUM:
        avio_printf(io, "%" PRIu64, e->samples_encoded);
        continue;
      ...
      case ENC_STATS_PKT_SIZE:
        ...
        continue;
      case ENC_STATS_KEYFRAME:
        avio_write(io, (pkt->flags & AV_PKT_FLAG_KEY) ? "K" : "N", 1);
        continue;
      ...
      case ENC_STATS_AVG_BITRATE: {
        ...
        avio_printf(io, "%g",
                    duration > 0 ? 8.0 * ep->data_size / duration : -1.);
        continue;
      }
      default:
        av_assert0(0);
...
static int encode_frame(OutputFile *of, OutputStream *ost, AVFrame *frame,
  ...
  if (frame) {
    ...
    if (ost->enc_stats_pre.io)
      enc_stats_write(ost, &ost->enc_stats_pre, frame, NULL, e->frames_encoded);

    e->frames_encoded++;
    e->samples_encoded += frame->nb_samples;

    if (debug_ts) {
      av_log(e, AV_LOG_INFO,
             "encoder <- type:%s "
             "frame_pts:%s frame_pts_time:%s time_base:%d/%d\n",
             type_desc, av_ts2str(frame->pts),
             av_ts2timestr(frame->pts, &enc->time_base), enc->time_base.num,
             enc->time_base.den);
    }
  ...
  while (1) {
    ...
    update_benchmark("%s_%s %d.%d", action, type_desc, of->index, ost->index);
    ...
    if (debug_ts) {
      av_log(e, AV_LOG_INFO,
             ...
             av_ts2timestr(pkt->pts, &enc->time_base), av_ts2str(pkt->dts),
             av_ts2timestr(pkt->dts, &enc->time_base), av_ts2str(pkt->duration),
...
static enum AVPictureType forced_kf_apply(void *logctx, KeyframeForceCtx *kf,
  ...
  if (kf->index < kf->nb_pts &&
      av_compare_ts(frame->pts, frame->time_base, kf->pts[kf->index],
                    AV_TIME_BASE_Q) >= 0) {
    kf->index++;
    goto force_keyframe;
  } else if (kf->pexpr) {
    ...
    av_log(logctx, AV_LOG_TRACE,
           ...
           kf->expr_const_values[FKF_N], kf->expr_const_values[FKF_N_FORCED],
...
static int frame_encode(OutputStream *ost, AVFrame *frame, AVPacket *pkt) {
  ...
  if (type == AVMEDIA_TYPE_SUBTITLE) {
    const AVSubtitle *subtitle =
        frame && frame->buf[0] ? (AVSubtitle *)frame->buf[0]->data : NULL;
