...
static void sub2video_heartbeat(InputStream *ist, int64_t pts) {
  ...
  for (i = 0; i < infile->nb_streams; i++) {
    ...
    if (pts2 >= ist2->sub2video.end_pts ||
        (!ist2->sub2video.frame->data[0] &&
         ist2->sub2video.end_pts < INT64_MAX))
...
void term_exit(void) { term_exit_sigsafe(); }
  ...
  for (i = 0; i < nb_filtergraphs; i++) {
    ...
    for (j = 0; j < fg->nb_inputs; j++) {
      while (av_fifo_size(fg->inputs[j]->frame_queue)) {
        AVFrame *frame;
        av_fifo_generic_read(fg->inputs[j]->frame_queue, &frame, sizeof(frame),
                             NULL);
        av_frame_free(&frame);
      }
      ...
      if (fg->inputs[j]->ist->sub2video.sub_queue) {
        while (av_fifo_size(fg->inputs[j]->ist->sub2video.sub_queue)) {
          AVSubtitle sub;
          av_fifo_generic_read(fg->inputs[j]->ist->sub2video.sub_queue, &sub,
                               sizeof(sub), NULL);
          avsubtitle_free(&sub);
        }
...
static void write_packet(OutputFile *of, AVPacket *pkt, OutputStream *ost,
  ...
  if (ret < 0) {
    ...
    main_return_code = 1;
    close_all_output_streams(ost, MUXER_FINISHED | ENCODER_FINISHED,
                             ENCODER_FINISHED);
  }
  av_packet_unref(pkt);
}
...
static void close_output_stream(OutputStream *ost) {
  ...
  if (of->shortest) {
    int64_t end = av_rescale_q(ost->sync_opts - ost->first_pts,
                               ost->enc_ctx->time_base, AV_TIME_BASE_Q);
...
static int check_recording_time(OutputStream *ost) {
  ...
  if (of->recording_time != INT64_MAX &&
      av_compare_ts(ost->sync_opts - ost->first_pts, ost->enc_ctx->time_base,
                    of->recording_time, AV_TIME_BASE_Q) >= 0) {
...
static void do_subtitle_out(OutputFile *of, OutputStream *ost,
                            AVSubtitle *sub) {
  ...
  for (i = 0; i < nb; i++) {
    ...
    sub->pts += av_rescale_q(sub->start_display_time, (AVRational){1, 1000},
                             AV_TIME_BASE_Q);
...
static void do_video_out(OutputFile *of, OutputStream *ost,
                         AVFrame *next_picture, double sync_ipts) {
  ...
  if (!ost->filters_script && !ost->filters && next_picture && ist &&
  ...
  if (!next_picture) {
    ...
    delta0 =
        sync_ipts -
        ost->sync_opts; // delta0 is the "drift" between the input frame
                        // (next_picture) and where it would fall in the output.
        ...
        format_video_sync =
            (of->ctx->oformat->flags & AVFMT_VARIABLE_FPS)
                ? ((of->ctx->oformat->flags & AVFMT_NOTIMESTAMPS)
                       ...
                       : VSYNC_VFR)
                : VSYNC_CFR;
      if (ist && format_video_sync == VSYNC_CFR &&
    ...
    ost->is_cfr =
        (format_video_sync == VSYNC_CFR || format_video_sync == VSYNC_VSCFR);

    if (delta0 < 0 && delta > 0 && format_video_sync != VSYNC_PASSTHROUGH &&
...

    {
      ...
      if (in_picture->interlaced_frame) {
        if (enc->codec->id == AV_CODEC_ID_MJPEG)
          mux_par->field_order =
              in_picture->top_field_first ? AV_FIELD_TT : AV_FIELD_BB;
        else
          mux_par->field_order =
              in_picture->top_field_first ? AV_FIELD_TB : AV_FIELD_BT;
      } else
      ...
      if (debug_ts) {
        av_log(NULL, AV_LOG_INFO,
               "encoder <- type:video "
               "frame_pts:%s frame_pts_time:%s time_base:%d/%d\n",
               av_ts2str(in_picture->pts),
               av_ts2timestr(in_picture->pts, &enc->time_base),
               enc->time_base.num, enc->time_base.den);
      }
...
static double psnr(double d) { return -10.0 * log10(d); }
...
static void finish_output_stream(OutputStream *ost) {
  ...
  if (of->shortest) {
    for (i = 0; i < of->ctx->nb_streams; i++)
      output_streams[of->ost_index + i]->finished =
          ENCODER_FINISHED | MUXER_FINISHED;
  }
}
...
static int reap_filters(int flush) {
  ...
  for (i = 0; i < nb_output_streams; i++) {
    ...
    while (1) {
      double float_pts =
          AV_NOPTS_VALUE; // this is identical to filtered_frame.pts but with
                          // higher precision
      ...
      if (filtered_frame->pts != AV_NOPTS_VALUE) {
        ...
        tb.den <<= extra_bits;
        float_pts = av_rescale_q(filtered_frame->pts, filter_tb, tb) -
      ...
      // if (ost->source_index >= 0)
      //     *filtered_frame= *input_streams[ost->source_index]->decoded_frame;
      //     //for me_threshold
      ...
      case AVMEDIA_TYPE_VIDEO:
        ...
        if (debug_ts) {
          av_log(NULL, AV_LOG_INFO,
                 ...
                 av_ts2timestr(filtered_frame->pts, &enc->time_base), float_pts,
                 enc->time_base.num, enc->time_base.den);
        }
...
static void print_final_stats(int64_t total_size) {
  ...
  for (i = 0; i < nb_input_files; i++) {
    ...
    for (j = 0; j < f->nb_streams; j++) {
      ...
      av_log(NULL, AV_LOG_VERBOSE, "  Input stream #%d:%d (%s): ", i, j,
  ...
  for (i = 0; i < nb_output_files; i++) {
    ...
    for (j = 0; j < of->ctx->nb_streams; j++) {
      ...
      av_log(NULL, AV_LOG_VERBOSE, "  Output stream #%d:%d (%s): ", i, j,
             media_type_string(type));
...
static void print_report(int is_last_report, int64_t timer_start,
  ...
  for (i = 0; i < nb_output_streams; i++) {
    ...
    if (!vid && enc->codec_type == AVMEDIA_TYPE_VIDEO) {
      ...
      av_bprintf(&buf, "frame=%5d fps=%3.*f q=%3.1f ", frame_number, fps < 9.95,
                 fps, q);
...
static void do_streamcopy(InputStream *ist, OutputStream *ost,
  ...
  int64_t ost_tb_start_time =
      av_rescale_q(start_time, AV_TIME_BASE_Q, ost->mux_timebase);
  AVPacket opkt = {0};
  ...
  if (!ost->frame_number && !ost->copy_prior_start) {
    ...
    if (pkt->pts == AV_NOPTS_VALUE
            ? ist->pts < comp_start
            : pkt->pts <
                  av_rescale_q(comp_start, AV_TIME_BASE_Q, ist->st->time_base))
      return;
  }
  ...
  if (ost->st->codecpar->codec_type == AVMEDIA_TYPE_AUDIO &&
    ...
    opkt.dts = opkt.pts =
        av_rescale_delta(ist->st->time_base, pkt->dts,
...
static int transcode_subtitles(InputStream *ist, AVPacket *pkt, int *got_output,
  ...
  int free_sub = 1;
  int i,
      ret = avcodec_decode_subtitle2(ist->dec_ctx, &subtitle, got_output, pkt);
  ...
  if (ist->sub2video.frame) {
    ...
    av_fifo_generic_write(ist->sub2video.sub_queue, &subtitle, sizeof(subtitle),
                          NULL);
    free_sub = 0;
  }
...
static int process_input_packet(InputStream *ist, const AVPacket *pkt,
  ...
  AVPacket avpkt;
  if (!ist->saw_first_ts) {
    ist->dts = ist->st->avg_frame_rate.num
                   ? -ist->dec_ctx->has_b_frames * AV_TIME_BASE /
  ...
  while (ist->decoding_needed) {
    ...
    case AVMEDIA_TYPE_VIDEO:
      ...
      if (got_output) {
        if (duration_pts > 0) {
          ist->next_pts +=
              av_rescale_q(duration_pts, ist->st->time_base, AV_TIME_BASE_Q);
        } else {
  ...
  if (!ist->decoding_needed && pkt) {
    ...
    case AVMEDIA_TYPE_VIDEO:
      if (ist->framerate.num) {
        ...
        int64_t next_dts =
            av_rescale_q(ist->next_dts, time_base_q, av_inv_q(ist->framerate));
        ist->next_dts =
    ...
    if (ist->dec_ctx->codec_id == AV_CODEC_ID_DVB_SUBTITLE &&
      ...
      if (ist->decoding_needed & DECODING_FOR_FILTER)
        av_log(NULL, AV_LOG_WARNING,
               ...
               "same time is not fully supported, "
               "also see -compute_edt [0|1]\n");
    }
...
static int init_output_bsfs(OutputStream *ost) {
  ...
  for (i = 0; i < ost->nb_bitstream_filters; i++) {
    ...
    ret = avcodec_parameters_copy(ctx->par_in, i ? ost->bsf_ctx[i - 1]->par_out
                                                 : ost->st->codecpar);
    if (ret < 0)
      return ret;

    ctx->time_base_in =
        i ? ost->bsf_ctx[i - 1]->time_base_out : ost->st->time_base;
...
static int init_output_stream_streamcopy(OutputStream *ost) {
  ...
  ret = avformat_transfer_internal_stream_timing_info(of->ctx->oformat, ost->st,
                                                      ist->st, copy_tb);
  if (ret < 0)
    return ret;
...
static int init_output_stream_encode(OutputStream *ost) {
  ...
  if (ist) {
    ...
    for (j = 0; j < oc->nb_streams; j++) {
      ...
      if (st != ost->st &&
          st->codecpar->codec_type == ost->st->codecpar->codec_type)
        break;
    }
    if (j == oc->nb_streams)
      if (ost->st->codecpar->codec_type == AVMEDIA_TYPE_AUDIO ||
  ...
  case AVMEDIA_TYPE_VIDEO:
    ...
    for (j = 0; j < ost->forced_kf_count; j++)
      ost->forced_kf_pts[j] = av_rescale_q(ost->forced_kf_pts[j],
                                           AV_TIME_BASE_Q, enc_ctx->time_base);
    ...
    if (ost->forced_keyframes) {
      if (!strncmp(ost->forced_keyframes, "expr:", 5)) {
        ret = av_expr_parse(
            &ost->forced_keyframes_pexpr, ost->forced_keyframes + 5,
  ...
  if (ost->encoding_needed) {
    ...
    if (ost->filter && av_buffersink_get_hw_frames_ctx(ost->filter->filter) &&
        ((AVHWFramesContext *)av_buffersink_get_hw_frames_ctx(
             ost->filter->filter)
             ->data)
                ->format == av_buffersink_get_format(ost->filter->filter)) {
  ...
  if (ost->disposition) {
    static const AVOption opts[] = {
        ...
        {"default",
         ...
         0,
         AV_OPT_TYPE_CONST,
         {.i64 = AV_DISPOSITION_DEFAULT},
         .unit = "flags"},
        {"dub",
         NULL,
         0,
         AV_OPT_TYPE_CONST,
         {.i64 = AV_DISPOSITION_DUB},
         .unit = "flags"},
        {"original",
         NULL,
         0,
         AV_OPT_TYPE_CONST,
         {.i64 = AV_DISPOSITION_ORIGINAL},
         .unit = "flags"},
        {"comment",
         NULL,
         0,
         AV_OPT_TYPE_CONST,
         {.i64 = AV_DISPOSITION_COMMENT},
         .unit = "flags"},
        {"lyrics",
         NULL,
         0,
         AV_OPT_TYPE_CONST,
         {.i64 = AV_DISPOSITION_LYRICS},
         .unit = "flags"},
        {"karaoke",
         NULL,
         0,
         AV_OPT_TYPE_CONST,
         {.i64 = AV_DISPOSITION_KARAOKE},
         .unit = "flags"},
        {"forced",
         NULL,
         0,
         AV_OPT_TYPE_CONST,
         {.i64 = AV_DISPOSITION_FORCED},
         .unit = "flags"},
        {"hearing_impaired",
         NULL,
         0,
         AV_OPT_TYPE_CONST,
         {.i64 = AV_DISPOSITION_HEARING_IMPAIRED},
         .unit = "flags"},
        {"visual_impaired",
         NULL,
         0,
         AV_OPT_TYPE_CONST,
         {.i64 = AV_DISPOSITION_VISUAL_IMPAIRED},
         .unit = "flags"},
        {"clean_effects",
         NULL,
         0,
         AV_OPT_TYPE_CONST,
         {.i64 = AV_DISPOSITION_CLEAN_EFFECTS},
         .unit = "flags"},
        {"attached_pic",
         NULL,
         0,
         AV_OPT_TYPE_CONST,
         {.i64 = AV_DISPOSITION_ATTACHED_PIC},
         .unit = "flags"},
        {"captions",
         NULL,
         0,
...
dump_format:
    ...
    for (j = 0; j < ist->nb_filters; j++) {
      if (!filtergraph_is_simple(ist->filters[j]->graph)) {
        av_log(NULL, AV_LOG_INFO, "  Stream #%d:%d (%s) -> %s", ist->file_index,
               ist->st->index, ist->dec ? ist->dec->name : "?",
               ist->filters[j]->name);
  ...
  for (i = 0; i < nb_output_streams; i++) {
    ...
    if (ost->sync_ist != input_streams[ost->source_index])
      av_log(NULL, AV_LOG_INFO, " [sync #%d:%d]", ost->sync_ist->file_index,
        ...
        if (fg->graph) {
          if (time < 0) {
            ret = avfilter_graph_send_command(
                fg->graph, target, command, arg, buf, sizeof(buf),
                key == 'c' ? AVFILTER_CMD_FLAG_ONE : 0);
            fprintf(stderr, "Command reply for stream %d: ret:%d res:\n%s", i,
                    ret, buf);
          } else if (key == 'c') {
            fprintf(stderr, "Queuing commands only on filters supporting the "
                            "specific command is unsupported\n");
            ret = AVERROR_PATCHWELCOME;
          } else {
            ret = avfilter_graph_queue_command(fg->graph, target, command, arg,
                                               0, time);
            if (ret < 0)
...
static int init_input_thread(int i) {
  ...
  if (f->ctx->pb ? !f->ctx->pb->seekable
    ...
    f->non_blocking = 1;
  ret = av_thread_message_queue_alloc(&f->in_thread_queue, f->thread_queue_size,
                                      sizeof(AVPacket));
  if (ret < 0)
    return ret;
  ...
  if ((ret = pthread_create(&f->thread, NULL, input_thread, f))) {
    av_log(NULL, AV_LOG_ERROR,
           "pthread_create failed: %s. Try to increase `ulimit -v` or decrease "
           "`ulimit -s`.\n",
           strerror(ret));
    av_thread_message_queue_free(&f->in_thread_queue);
    return AVERROR(ret);
...
static int get_input_packet_mt(InputFile *f, AVPacket *pkt) {
  return av_thread_message_queue_recv(
      f->in_thread_queue, pkt,
...
static AVRational duration_max(int64_t tmp, int64_t *duration,
...
static int process_input(int file_index) {
  ...
  if (debug_ts) {
    av_log(
        ...
        "pkt_pts_time:%s pkt_dts:%s "
        "pkt_dts_time:%s off:%s off_time:%s\n",
  ...
  if (pkt.dts != AV_NOPTS_VALUE)
    pkt.dts +=
        av_rescale_q(ifile->ts_offset, AV_TIME_BASE_Q, ist->st->time_base);
  if (pkt.pts != AV_NOPTS_VALUE)
    pkt.pts +=
        av_rescale_q(ifile->ts_offset, AV_TIME_BASE_Q, ist->st->time_base);
  ...
  if (debug_ts) {
    av_log(NULL, AV_LOG_INFO,
           ...
           "pkt_dts:%s pkt_dts_time:%s off:%s "
           "off_time:%s\n",
           ifile->ist_index + pkt.stream_index,
           av_get_media_type_string(ist->dec_ctx->codec_type),
           av_ts2str(pkt.pts), av_ts2timestr(pkt.pts, &ist->st->time_base),
...
int exe_ffmpeg_cmd(int argc, char **argv) {
  ...
  if (savedCode == 0) {
    ...
    av_log(NULL, AV_LOG_DEBUG,
           "%" PRIu64 " frames successfully decoded, %" PRIu64
           " decoding errors\n",
