...
void log_call_back_with_callback(void *ptr, int level, const char *fmt, va_list vl) {
  char buffer[8192];
  ...
  if (g_currentCallbackInfo != nullptr && g_currentCallbackInfo->hasLogCallback && 
      g_currentCallbackInfo->utilsClass != nullptr && g_currentCallbackInfo->sendLogMethodID != nullptr) {
    JNIEnv *env = getJNIEnv();
    if (env != nullptr) {
      jstring logMessage = env->NewStringUTF(buffer);
      env->CallStaticVoidMethod(g_currentCallbackInfo->utilsClass, g_currentCallbackInfo->sendLogMethodID, level, logMessage);
      env->DeleteLocalRef(logMessage);
    }
  }
  
  // 检查是否是进度信息（FFmpeg 的进度输出格式：size=... time=... bitrate=... speed=...）
  if (g_currentCallbackInfo != nullptr && g_currentCallbackInfo->hasProgressCallback && 
      g_currentCallbackInfo->utilsClass != nullptr && g_currentCallbackInfo->sendProgressMethodID != nullptr) {
    std::string logLine(buffer);
    // 检查是否包含进度信息的关键字
    if (logLine.find("size=") != std::string::npos && 
        (logLine.find("time=") != std::string::npos || logLine.find("bitrate=") != std::string::npos)) {
      // 提取进度信息
      std::string progressMsg = logLine;
      // 清理换行符和多余空格
      while (!progressMsg.empty() && (progressMsg.back() == '\n' || progressMsg.back() == '\r' || progressMsg.back() == ' ')) {
      ...
      if (!progressMsg.empty()) {
        ...
        if (env != nullptr) {
          ...
          env->CallStaticVoidMethod(g_currentCallbackInfo->utilsClass, g_currentCallbackInfo->sendProgressMethodID, message);
...
void progressCallBack(int64_t handle, int what, float progress) {
  if (handle != 0) {
    ...
    if (onActionListener->hasProgressCallback && onActionListener->utilsClass != nullptr && 
        onActionListener->sendProgressMethodID != nullptr) {
      char buffer[256];
      snprintf(buffer, sizeof(buffer), "Progress: %.2f%%", progress * 100.0f);
      jstring message = env->NewStringUTF(buffer);
      env->CallStaticVoidMethod(onActionListener->utilsClass, onActionListener->sendProgressMethodID, message);
      env->DeleteLocalRef(message);
    }
  }
}
...
// 新增：创建 Session
extern "C" JNIEXPORT jlong JNICALL
Java_com_i7play_tiny_1ffmpeg_FFMpegUtils_createFFmpegSession(
...
Java_com_i7play_tiny_1ffmpeg_FFMpegUtils_executeFFmpegCommandWithSession(
  ...
  if (NULL != actionCallBack) {
    ...
    extern int executeFFmpegCommandWithSession(int64_t sessionId, int argc, char **argv,
                                               int64_t handle,
    ...
    if (ret != 0) {
      ...
      if (strlen(detailedError) > 0) {
        snprintf(fullErrorMsg, sizeof(fullErrorMsg), 
                "Error code: %d (%s)\n\nDetailed error log:\n%s", 
                ret, err, detailedError);
      } else {
        snprintf(fullErrorMsg, sizeof(fullErrorMsg), 
                "Error code: %d (%s)", ret, err);
      }
    ...
    extern int executeFFmpegCommandWithSession(int64_t sessionId, int argc, char **argv,
                                               ...
                                               void (*progressCallBack)(int64_t, int, float),
                                               int64_t totalTime);
    ret = executeFFmpegCommandWithSession(sessionId, cmdLen, argCmd, 0, NULL, totalTime);
  }
...
Java_com_i7play_tiny_1ffmpeg_FFMpegUtils_destroyFFmpegSession(
    JNIEnv *env, jclass clazz, jlong sessionId) {
...
// 新增：设置日志输出开关
extern "C" JNIEXPORT void JNICALL
Java_com_i7play_tiny_1ffmpeg_FFMpegUtils_setLogEnabled(
