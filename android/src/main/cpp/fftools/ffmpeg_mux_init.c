...
static MuxStream *mux_stream_alloc(Muxer *mux, enum AVMediaType type) {
  ...
  ms->ost.av_class = &output_stream_class;
...
static int ost_get_filters(const OptionsContext *o, AVFormatContext *oc,
  ...
  opt_match_per_stream_str(ost, &o->filter_scripts, oc, ost->st,
                           &filters_script);
#endif
...
static enum AVPixelFormat choose_pixel_fmt(const AVCodecContext *avctx,
  ...
  // FIXME: This should check for AV_PIX_FMT_FLAG_ALPHA after PAL8 pixel format
  // without alpha is implemented
  int has_alpha = desc ? desc->nb_components % 2 == 0 : 0;
  enum AVPixelFormat best = AV_PIX_FMT_NONE;
  int ret;

  ret = avcodec_get_supported_config(avctx, NULL, AV_CODEC_CONFIG_PIX_FORMAT, 0,
                                     (const void **)&p, NULL);
  if (ret < 0)
  ...
  if (*p == AV_PIX_FMT_NONE) {
    if (target != AV_PIX_FMT_NONE)
      av_log(NULL, AV_LOG_WARNING,
             ...
             "format '%s'\n",
             av_get_pix_fmt_name(target), avctx->codec->name,
...
static enum AVPixelFormat pix_fmt_parse(OutputStream *ost, const char *name) {
  ...
  ret = avcodec_get_supported_config(ost->enc->enc_ctx, NULL,
...
static int new_stream_video(Muxer *mux, const OptionsContext *o,
  ...
  AVStream *st;
  const char *frame_rate = NULL, *max_frame_rate = NULL,
  ...
  if (max_frame_rate &&
      av_parse_video_rate(&ms->max_frame_rate, max_frame_rate) < 0) {
  ...
  opt_match_per_stream_str(ost, &o->frame_aspect_ratios, oc, st,
                           &frame_aspect_ratio);
  ...
  if (ost->enc) {
    ...
    if (chroma_intra_matrix) {
      if (!(video_enc->chroma_intra_matrix =
        ...
        return AVERROR(ENOMEM);
      ret = parse_matrix_coeffs(ost, video_enc->chroma_intra_matrix,
                                chroma_intra_matrix);
      if (ret < 0)
        return ret;
    }
    ...
    opt_match_per_stream_str(ost, &o->passlogfiles, oc, st,
                             &ost->logfile_prefix);
    ...
    if (do_pass) {
      ...
      snprintf(logfilename, sizeof(logfilename), "%s-%d.log",
               ...
               ost_idx);
      if (!strcmp(video_enc->codec->name, "libx264") ||
          !strcmp(video_enc->codec->name, "libvvenc")) {
        if (av_opt_is_set_to_default_by_name(video_enc, "stats",
                                             AV_OPT_SEARCH_CHILDREN) > 0)
          av_opt_set(video_enc, "stats", logfilename, AV_OPT_SEARCH_CHILDREN);
      } else {
    ...
    opt_match_per_stream_int(ost, &o->top_field_first, oc, st,
                             &ost->top_field_first);
    ...
    *vsync_method = VSYNC_AUTO;
    ...
    if (fps_mode) {
      ret = parse_and_set_vsync(fps_mode, vsync_method, ost->file->index,
                                ost->index, 0);
      if (ret < 0)
        return ret;
    }
    ...
    if ((ms->frame_rate.num || ms->max_frame_rate.num) &&
        !(*vsync_method == VSYNC_AUTO || *vsync_method == VSYNC_CFR ||
...
static int new_stream_audio(Muxer *mux, const OptionsContext *o,
  ...
  if (ost->enc) {
    ...
    if (layout &&
        av_channel_layout_from_string(&audio_enc->ch_layout, layout) < 0) {
    ...
    opt_match_per_stream_int(ost, &o->audio_sample_rate, oc, st,
                             &audio_enc->sample_rate);
...
static int new_stream_subtitle(Muxer *mux, const OptionsContext *o,
  ...
  if (ost->enc) {
    ...
    if (frame_size) {
      int ret = av_parse_video_size(&subtitle_enc->width, &subtitle_enc->height,
                                    frame_size);
      if (ret < 0) {
        av_log(ost, AV_LOG_FATAL, "Invalid frame size: %s.\n", frame_size);
        return ret;
...
static int ost_bind_filter(const Muxer *mux, MuxStream *ms,
                           OutputFilter *ofilter, const OptionsContext *o,
  ...
  OutputFilterOptions opts = {
      ...
      .flags = OFILTER_FLAG_DISABLE_CONVERT * !!keep_pix_fmt |
               ...
               OFILTER_FLAG_AUDIO_24BIT *
                   !!(av_get_exact_bits_per_sample(enc_ctx->codec_id) == 24),
  };
  ...
  if (ost->type == AVMEDIA_TYPE_VIDEO) {
    ...
    if (!ms->force_fps) {
      ret = avcodec_get_supported_config(
          enc_ctx, NULL, AV_CODEC_CONFIG_FRAME_RATE, 0,
  ...
  if (ofilter) {
    ...
    ret = fg_create_simple(&ost->fg_simple, ost->ist, filters, mux->sch,
                           ms->sch_idx_enc, &opts);
    if (ret >= 0)
...
static int streamcopy_init(const OptionsContext *o, const Muxer *mux,
  ...
  if (ret < 0) {
    av_log(ost, AV_LOG_FATAL, "Error getting reference codec parameters.\n");
    goto fail;
  }
...
static int ost_add(Muxer *mux, const OptionsContext *o, enum AVMediaType type,
                   ...
                   const ViewSpecifier *vs, OutputStream **post) {
  AVFormatContext *oc = mux->fc;
  MuxStream *ms;
  OutputStream *ost;
  const AVCodec *enc;
  ...
  if (ist)
    av_log(ost, AV_LOG_VERBOSE, "input stream %d:%d", ist->file->index,
           ist->index);
  else if (ofilter)
  ...
  if (ost->enc) {
    ...
    ret = filter_codec_opts(o->g->codec_opts, enc->id, oc, st, enc,
                            &encoder_opts, &mux->enc_opts_used);
    if (ret < 0)
      goto fail;
    ...
    if (enc_stats_post &&
      ...
      ret =
          enc_stats_init(ost, &ost->enc_stats_post, 0, enc_stats_post, format);
      if (ret < 0)
        goto fail;
    }
    ...
    ret = av_opt_set_dict2(ost->enc->enc_ctx, &encoder_opts,
    ...
    ret = filter_codec_opts(o->g->codec_opts, AV_CODEC_ID_NONE, oc, st, NULL,
                            &encoder_opts, &mux->enc_opts_used);
    if (ret < 0)
      goto fail;
  }
  ...
  opt_match_per_stream_int(ost, &o->copy_prior_start, oc, st,
                           &ms->copy_prior_start);
  ...
  if (ms->sch_idx >= 0) {
    ...
    opt_match_per_stream_int(ost, &o->muxing_queue_data_threshold, oc, st,
  ...
  opt_match_per_stream_int(ost, &o->fix_sub_duration_heartbeat, oc, st,
  ...
  opt_match_per_stream_int(ost, &o->copy_initial_nonkeyframes, oc, st,
                           &ms->copy_initial_nonkeyframes);
  switch (type) {
  case AVMEDIA_TYPE_VIDEO:
    ret = new_stream_video(mux, o, ost, &keep_pix_fmt, &vsync_method);
    break;
  case AVMEDIA_TYPE_AUDIO:
  ...
  if (ost->enc && (type == AVMEDIA_TYPE_VIDEO || type == AVMEDIA_TYPE_AUDIO)) {
    ...
    if (ost->enc) {
      ret = sch_connect(mux->sch, src, SCH_ENC(ms->sch_idx_enc));
      if (ret < 0)
        goto fail;
      src = SCH_ENC(ms->sch_idx_enc);
    }
  ...
  if (src.type != SCH_NODE_TYPE_NONE) {
    ret = sch_connect(mux->sch, src, SCH_MSTREAM(mux->sch_idx, ms->sch_idx));
    if (ret < 0)
      goto fail;
  } else {
...
static int map_auto_audio(Muxer *mux, const OptionsContext *o) {
  ...
  for (int j = 0; j < nb_input_files; j++) {
    ...
    if (file_best_ist) {
      file_best_score -=
          5000000 * !!(file_best_ist->st->disposition & AV_DISPOSITION_DEFAULT);
      if (file_best_score > best_score) {
        best_score = file_best_score;
        best_ist = file_best_ist;
      }
  ...
  for (InputStream *ist = ist_iter(NULL); ist; ist = ist_iter(ist))
    if (ist->st->codecpar->codec_type == AVMEDIA_TYPE_SUBTITLE) {
      ...
      if (subtitle_codec_name || input_props & output_props ||
          // Map dvb teletext which has neither property to any output subtitle
          // encoder
...
static int map_manual(Muxer *mux, const OptionsContext *o,
  ...
  loop_end:
    ...
    const ViewSpecifier *vs =
        map->vs.type == VIEW_SPECIFIER_TYPE_NONE ? NULL : &map->vs;
...
static int of_add_attachments(Muxer *mux, const OptionsContext *o) {
  ...
  read_fail:
    ...
    av_dict_set(&ost->st->metadata, "filename",
                (p && *p) ? p + 1 : o->attachments[i], AV_DICT_DONT_OVERWRITE);
  }
...
#define IS_INTERLEAVED(type) (type != AVMEDIA_TYPE_ATTACHMENT)
  ...
  for (int i = 0; i < oc->nb_streams; i++) {
    ...
    nb_audio_fs += (ost->enc && type == AVMEDIA_TYPE_AUDIO &&
                    !(ost->enc->enc_ctx->codec->capabilities &
  ...
  if (nb_interleaved > nb_av_enc) {
    ...
    for (int i = 0; i < oc->nb_streams; i++) {
      ...
      ms->sq_idx_mux =
          sq_add_stream(mux->sq_mux, shortest || ms->max_frames < INT64_MAX);
      if (ms->sq_idx_mux < 0)
...
static int of_parse_iamf_audio_element_layers(Muxer *mux, AVStreamGroup *stg,
  ...
  audio_element->demixing_info = av_iamf_param_definition_alloc(
      AV_IAMF_PARAMETER_DEFINITION_DEMIXING, 1, NULL);
  audio_element->recon_gain_info = av_iamf_param_definition_alloc(
      AV_IAMF_PARAMETER_DEFINITION_RECON_GAIN, 1, NULL);

  if (!audio_element->demixing_info || !audio_element->recon_gain_info)
    return AVERROR(ENOMEM);
...
static int of_parse_iamf_submixes(Muxer *mux, AVStreamGroup *stg, char *ptr) {
  ...
  while (token) {
    ...
    submix->output_mix_config = av_iamf_param_definition_alloc(
        AV_IAMF_PARAMETER_DEFINITION_MIX_GAIN, 0, NULL);
    ...
    while (subtoken) {
      ...
      if (element) {
        ...
        submix_element->element_mix_config = av_iamf_param_definition_alloc(
            AV_IAMF_PARAMETER_DEFINITION_MIX_GAIN, 0, NULL);
...
static int of_serialize_options(Muxer *mux, void *obj, AVBPrint *bp) {
  ...
  ret = av_opt_serialize(
      obj, 0, AV_OPT_SERIALIZE_SKIP_DEFAULTS | AV_OPT_SERIALIZE_SEARCH_CHILDREN,
      &ptr, '=', ':');
  if (ret < 0) {
...
#define SERIALIZE_LOOP_SUBBLOCK(obj)                                           \
  do {                                                                         \
    for (int k = 0; k < obj->nb_subblocks; k++) {                              \
      ret = of_serialize_options(                                              \
          mux, av_iamf_param_definition_get_subblock(obj, k), bp);             \
      if (ret < 0)                                                             \
        return ret;                                                            \
    }                                                                          \
  } while (0)
...
static int of_map_group(Muxer *mux, AVDictionary **dict, AVBPrint *bp,
                        const char *map) {
  AVStreamGroup *stg;
  ...
  case AV_STREAM_GROUP_PARAMS_IAMF_MIX_PRESENTATION: {
    ...
    for (int i = 0; i < mix->nb_submixes; i++) {
      ...
      for (int j = 0; j < submix->nb_elements; j++) {
        ...
        int64_t id =
            get_stream_group_index_from_id(mux, element->audio_element_id);
  ...
  while (e = av_dict_get(dict, "stg", e, 0)) {
    ...
    for (unsigned i = 0; i < oc->stream_groups[idx]->nb_streams; i++) {
      ret = avformat_stream_group_add_stream(
          stg, oc->stream_groups[idx]->streams[i]);
      if (ret < 0)
        goto end;
    }
  }
...
static int of_add_metadata(OutputFile *of, AVFormatContext *oc,
  ...
  for (int i = 0; i < o->metadata.nb_opt; i++) {
    ...
    *val++ = 0;
    ...
    ret = parse_meta_type(of, o->metadata.opt[i].specifier, &type, &index,
                          &stream_spec);
    if (ret < 0)
      return ret;
    ...
    if (type == 's') {
      for (int j = 0; j < oc->nb_streams; j++) {
        if ((ret = check_stream_specifier(oc, oc->streams[j], stream_spec)) >
            0) {
          av_dict_set(&oc->streams[j]->metadata, o->metadata.opt[i].u.str,
                      *val ? val : NULL, 0);
        } else if (ret < 0)
          return ret;
      }
...
static int copy_chapters(InputFile *ifile, OutputFile *ofile,
  ...
  for (int i = 0; i < is->nb_chapters; i++) {
    ...
    int64_t ts_off = av_rescale_q(start_time - ifile->ts_offset, AV_TIME_BASE_Q,
                                  in_ch->time_base);
...
#define SET_DICT(type, meta, context, index)                                   \
  ...
  if (type_in == 's') {
    for (i = 0; i < ic->nb_streams; i++) {
      if ((ret = check_stream_specifier(ic, ic->streams[i], istream_spec)) >
          0) {
  ...
  if (type_out == 's') {
    for (i = 0; i < oc->nb_streams; i++) {
      if ((ret = check_stream_specifier(oc, oc->streams[i], ostream_spec)) >
          0) {
...
static int set_dispositions(Muxer *mux, const OptionsContext *o) {
  ...
  for (int i = 0; i < ctx->nb_streams; i++) {
    ...
    opt_match_per_stream_str(ost, &o->disposition, ctx, ost->st,
                             &dispositions[i]);
  ...
  for (i = 0; i < n; i++) {
    ...
    if (strstr(p, "chapters") == p) {
      ...
      if (p[8]) {
        ...
        if (ret < 0) {
          av_log(log, AV_LOG_ERROR, "Invalid chapter time offset: %s\n", p + 8);
          goto fail;
        }
      } else
        t = 0;
      ...
      for (j = 0; j < nb_ch; j++) {
        ...
        pts[index++] = av_rescale_q(c->start, c->time_base, AV_TIME_BASE_Q) + t;
      }
...
static int process_forced_keyframes(Muxer *mux, const OptionsContext *o) {
  for (int i = 0; i < mux->of.nb_streams; i++) {
    ...
    opt_match_per_stream_str(ost, &o->forced_key_frames, mux->fc, ost->st,
                             &forced_keyframes);

    if (!(ost->type == AVMEDIA_TYPE_VIDEO && ost->enc && forced_keyframes))
      continue;
    ...
    if (!strncmp(forced_keyframes, "expr:", 5)) {
      int ret = av_expr_parse(&ost->kf.pexpr, forced_keyframes + 5,
                              ...
                              NULL, 0, NULL);
      if (ret < 0) {
        av_log(ost, AV_LOG_ERROR, "Invalid force_key_frames expression '%s'\n",
               forced_keyframes + 5);
        return ret;
      }
...
static Muxer *mux_alloc(void) {
  ...
  mux->of.av_class = &output_file_class;
