...
#define DECODING_FOR_FILTER 2
  ...
  uint64_t data_size;
  // latest wallclock time at which packet reading resumed after a stall - used
  // for readrate
  int64_t resume_wc;
...
static void report_new_stream(Demuxer *d, const AVPacket *pkt) {
  ...
  av_log(d, AV_LOG_WARNING,
         ...
         av_get_media_type_string(st->codecpar->codec_type), pkt->stream_index,
...
static int seek_to_start(Demuxer *d, Timestamp end_pts) {
  ...
  if (d->max_pts.ts != AV_NOPTS_VALUE) {
    ...
    d->duration.ts =
        d->max_pts.ts - av_rescale_q(min_pts, d->min_pts.tb, d->max_pts.tb);
  }
...
static int ist_dts_update(DemuxStream *ds, AVPacket *pkt, FrameData *fd) {
  ...
  case AVMEDIA_TYPE_VIDEO:
    if (ist->framerate.num) {
      ...
      int64_t next_dts =
          av_rescale_q(ds->next_dts, time_base_q, av_inv_q(ist->framerate));
      ds->next_dts =
      ...
      if (ds->codec_desc && (ds->codec_desc->props & AV_CODEC_PROP_FIELDS) &&
...
#define SHOW_TS_DEBUG(tag_)                                                    \
  ...
  if (pkt->pts != AV_NOPTS_VALUE) {
    ...
    if (d->max_pts.ts == AV_NOPTS_VALUE ||
        av_compare_ts(d->max_pts.ts, d->max_pts.tb, pkt->pts + pkt_duration,
                      pkt->time_base) < 0) {
      d->max_pts =
          (Timestamp){.ts = pkt->pts + pkt_duration, .tb = pkt->time_base};
    }
    if (d->min_pts.ts == AV_NOPTS_VALUE ||
        av_compare_ts(d->min_pts.ts, d->min_pts.tb, pkt->pts, pkt->time_base) >
            0) {
...
static int input_packet_process(Demuxer *d, AVPacket *pkt,
  ...
  if (debug_ts) {
    av_log(ist, AV_LOG_INFO,
           ...
           av_get_media_type_string(ist->par->codec_type), av_ts2str(pkt->pts),
           av_ts2timestr(pkt->pts, &pkt->time_base), av_ts2str(pkt->dts),
           av_ts2timestr(pkt->dts, &pkt->time_base), av_ts2str(pkt->duration),
           av_ts2timestr(pkt->duration, &pkt->time_base),
           av_ts2str(f->ts_offset),
           av_ts2timestr(f->ts_offset, &AV_TIME_BASE_Q));
  }
...
static void readrate_sleep(Demuxer *d) {
  ...
  for (int i = 0; i < f->nb_streams; i++) {
    ...
    stream_ts_offset =
        FFMAX(ds->first_dts != AV_NOPTS_VALUE ? ds->first_dts : 0, file_start);
    ...
    if ((!ds->lag && lag > 0.3 * AV_TIME_BASE) ||
        (lag > ds->lag + 0.3 * AV_TIME_BASE)) {
      ds->lag = lag;
      ds->resume_wc = now;
      ds->resume_pts = pts;
      av_log_once(ds, AV_LOG_WARNING, AV_LOG_DEBUG, &resume_warn,
                  "Resumed reading at pts %0.3f with rate %0.3f after a lag of "
                  "%0.3fs\n",
...
static int input_thread(void *arg) {
  ...
  while (1) {
    ...
    ds = dt.pkt_demux->stream_index < f->nb_streams
             ? ds_from_ist(f->streams[dt.pkt_demux->stream_index])
             : NULL;
...
int ist_use(InputStream *ist, int decoding_needed, const ViewSpecifier *vs,
            SchedulerNode *src) {
  ...
  if (decoding_needed && ds->sch_idx_dec < 0) {
    ...
    ds->dec_opts.flags |=
        (!!ist->fix_sub_duration * DECODER_FLAG_FIX_SUB_DURATION) |
    ...
    if (ist->dec->id == AV_CODEC_ID_DVB_SUBTITLE &&
      ...
      av_dict_set(&ds->decoder_opts, "compute_edt", "1",
    ...
    snprintf(ds->dec_name, sizeof(ds->dec_name), "%d:%d", ist->file->index,
             ist->index);
    ...
    ret = dec_init(&ist->decoder, d->sch, &ds->decoder_opts, &ds->dec_opts,
                   ds->decoded_params);
    if (ret < 0)
      return ret;
    ds->sch_idx_dec = ret;
  ...
  if (decoding_needed && ist->par->codec_type == AVMEDIA_TYPE_VIDEO) {
    ...
    *src = decoding_needed ? SCH_DEC_OUT(ds->sch_idx_dec, 0)
...
int ist_filter_add(InputStream *ist, InputFilter *ifilter, int is_simple,
  ...
  ret =
      ist_use(ist, is_simple ? DECODING_FOR_OST : DECODING_FOR_FILTER, vs, src);
  if (ret < 0)
    return ret;
  ...
  if (ist->par->codec_type == AVMEDIA_TYPE_VIDEO) {
    const AVPacketSideData *sd = av_packet_side_data_get(
        ist->par->coded_side_data, ist->par->nb_coded_side_data,
    ...
    if (sd && sd->size >= sizeof(uint32_t) * 4) {
      ...
      if (ds->apply_cropping && ds->apply_cropping != CROP_CODEC &&
          (opts->crop_top | opts->crop_bottom | opts->crop_left |
           opts->crop_right))
  ...
  ret = av_channel_layout_copy(&opts->fallback->ch_layout,
                               &ds->decoded_params->ch_layout);
  if (ret < 0)
    return ret;
  ...
  opts->trim_start_us =
      ((d->f.start_time == AV_NOPTS_VALUE) || !d->accurate_seek)
          ? AV_NOPTS_VALUE
          : tsoffset;
...
static int
choose_decoder(const OptionsContext *o, void *logctx, AVFormatContext *s,
               AVStream *st, enum HWAccelID hwaccel_id,
               enum AVHWDeviceType hwaccel_device_type, const AVCodec **pcodec)
        ...
        for (int j = 0; config = avcodec_get_hw_config(c, j); j++) {
          if (config->device_type == hwaccel_device_type) {
            av_log(logctx, AV_LOG_VERBOSE,
                   "Selecting decoder '%s' because of requested hwaccel method "
                   "%s\n",
...
static DemuxStream *demux_stream_alloc(Demuxer *d, AVStream *st) {
  ...
  ds->ist.av_class = &input_stream_class;
...
static int ist_add(const OptionsContext *o, Demuxer *d, AVStream *st,
  ...
  if (st->codecpar->codec_type == AVMEDIA_TYPE_VIDEO) {
    ...
    if (!hwaccel_output_format && hwaccel && !strcmp(hwaccel, "cuvid")) {
      av_log(
          ...
          "with old commandlines. This behaviour is DEPRECATED and will be "
          "removed "
      ...
      ds->dec_opts.hwaccel_output_format =
  ...
  if (ist->dec) {
    ret = filter_codec_opts(o->g->codec_opts, ist->st->codecpar->codec_id, ic,
                            st, ist->dec, &ds->decoder_opts, opts_used);
    if (ret < 0)
      return ret;
  }
  ...
  if (ds->drop_changed && ds->reinit_filters) {
    if (ds->reinit_filters > 0) {
      av_log(ist, AV_LOG_ERROR,
             "drop_changed and reinit_filters both enabled. These are mutually "
             "exclusive.\n");
      return AVERROR(EINVAL);
    }
    ds->reinit_filters = 0;
  ...
  av_dict_set_int(&ds->decoder_opts, "apply_cropping",
                  ds->apply_cropping && ds->apply_cropping != CROP_CONTAINER,
                  0);
  ...
  case AVMEDIA_TYPE_VIDEO:
    ...
    opt_match_per_stream_int(ist, &o->top_field_first, ic, st,
                             &ist->top_field_first);
#endif
  ...
  case AVMEDIA_TYPE_AUDIO: {
    ...
    if (ch_layout_str) {
      ...
      if (par->ch_layout.nb_channels <= 0 ||
          par->ch_layout.nb_channels == ch_layout.nb_channels) {
        av_channel_layout_uninit(&par->ch_layout);
        par->ch_layout = ch_layout;
      } else {
        av_log(ist, AV_LOG_ERROR,
               "Specified channel layout '%s' has %d channels, but input has "
               "%d channels.\n",
               ch_layout_str, ch_layout.nb_channels,
      ...
      opt_match_per_stream_int(ist, &o->guess_layout_max, ic, st,
                               &guess_layout_max);
  ...
  case AVMEDIA_TYPE_SUBTITLE: {
    ...
    opt_match_per_stream_int(ist, &o->fix_sub_duration, ic, st,
...
int ifile_open(const OptionsContext *o, const char *filename, Scheduler *sch) {
  ...
  if (o->audio_ch_layouts.nb_opt) {
    ...
    if (file_iformat && (priv_class = file_iformat->priv_class) &&
      ...
      av_dict_set(&o->g->format_opts, "ch_layout",
                  o->audio_ch_layouts.opt[o->audio_ch_layouts.nb_opt - 1].u.str,
                  0);
    }
  }
  ...
  if (o->frame_sizes.nb_opt) {
    av_dict_set(&o->g->format_opts, "video_size",
                o->frame_sizes.opt[o->frame_sizes.nb_opt - 1].u.str, 0);
  }
  ...
  if (video_codec_name)
    ret = err_merge(ret, find_codec(NULL, video_codec_name, AVMEDIA_TYPE_VIDEO,
                                    0, &ic->video_codec));
  if (audio_codec_name)
    ret = err_merge(ret, find_codec(NULL, audio_codec_name, AVMEDIA_TYPE_AUDIO,
                                    0, &ic->audio_codec));
  if (subtitle_codec_name)
    ret = err_merge(ret,
                    find_codec(NULL, subtitle_codec_name, AVMEDIA_TYPE_SUBTITLE,
                               0, &ic->subtitle_codec));
  if (data_codec_name)
  ...
  for (int i = 0; i < ic->nb_streams; i++) {
    const AVCodec *dummy;
    ret = choose_decoder(o, f, ic, ic->streams[i], HWACCEL_NONE,
  ...
  if (d->readrate) {
    ...
    d->readrate_catchup =
        o->readrate_catchup ? o->readrate_catchup : d->readrate * 1.05;
    ...
    if (o->readrate_catchup) {
      av_log(d, AV_LOG_WARNING,
             "Option -readrate_catchup ignored "
             "since neither -readrate nor -re were given\n");
    }
  }
