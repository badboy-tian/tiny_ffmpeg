...
static void sub2video_update(InputFilterPriv *ifp, int64_t heartbeat_pts,
  ...
  if (sub) {
    ...
    pts = ifp->sub2video.initialize ? heartbeat_pts : ifp->sub2video.end_pts;
    end_pts = INT64_MAX;
    num_rects = 0;
  }
...
static int read_binary(void *logctx, const char *path, uint8_t **data,
                       int *len) {
  AVIOContext *io = NULL;
  int64_t fsize;
  int ret;
...
static int filter_opt_apply(void *logctx, AVFilterContext *f, const char *key,
                            const char *val) {
...
static int graph_parse(void *logctx, AVFilterGraph *graph, const char *desc,
...
static OutputFilter *ofilter_alloc(FilterGraph *fg, enum AVMediaType type) {
  ...
  ofilter = &ofp->ofilter;
  ofilter->av_class = &ofilter_class;
  ofp->log_parent = fg;
  ofilter->graph = fg;
  ofilter->type = type;
  ofp->format = -1;
...
static int ifilter_bind_ist(InputFilter *ifilter, InputStream *ist,
  ...
  ret = ist_filter_add(ist, ifilter, filtergraph_is_simple(ifilter->graph), vs,
                       &ifp->opts, &src);
  if (ret < 0)
    return ret;
  ...
  ret = sch_connect(fgp->sch, src, SCH_FILTER_IN(fgp->sch_idx, ifilter->index));
  if (ret < 0)
    return ret;
...
int ofilter_bind_enc(OutputFilter *ofilter, unsigned sched_idx_enc,
  ...
  av_assert0(!opts->enc || ofilter->type == opts->enc->type);
...
static int ifilter_bind_fg(InputFilterPriv *ifp, FilterGraph *fg_src,
                           int out_idx) {
  FilterGraphPriv *fgp = fgp_from_fg(ifp->ifilter.graph);
...
int fg_create(FilterGraph **pfg, char *graph_desc, Scheduler *sch) {
  ...
  for (AVFilterInOut *cur = inputs; cur; cur = cur->next) {
    ...
    if (ifilter->type != AVMEDIA_TYPE_VIDEO &&
        ifilter->type != AVMEDIA_TYPE_AUDIO) {
  ...
  ret = sch_add_filtergraph(sch, fg->nb_inputs, fg->nb_outputs, filter_thread,
                            fgp);
  if (ret < 0)
    goto fail;
  fgp->sch_idx = ret;
...
int fg_create_simple(FilterGraph **pfg, InputStream *ist, char *graph_desc,
...
int fg_finalise_bindings(void) {
  ...
  for (int i = 0; i < nb_filtergraphs; i++) {
    ...
    for (int j = 0; j < fg->nb_outputs; j++) {
      ...
      if (!output->bound) {
        av_log(fg, AV_LOG_FATAL, "Filter '%s' has output %d (%s) unconnected\n",
               output->name, j,
...
static int insert_filter(AVFilterContext **last_filter, int *pad_idx,
  ...
  ret = avfilter_graph_create_filter(&ctx, filter, filter_name, args, NULL,
                                     graph);
  if (ret < 0)
    return ret;
...
static int configure_output_filter(FilterGraphPriv *fgp, AVFilterGraph *graph,
  ...
  switch (ofilter->type) {
  case AVMEDIA_TYPE_VIDEO:
    return configure_output_video_filter(fgp, graph, ofilter, out);
  case AVMEDIA_TYPE_AUDIO:
    return configure_output_audio_filter(fgp, graph, ofilter, out);
  default:
    av_assert0(0);
    return 0;
  }
...
static int configure_input_video_filter(FilterGraph *fg, AVFilterGraph *graph,
  ...
  if ((ifp->opts.flags & IFILTER_FLAG_CROP)) {
    ...
    snprintf(crop_buf, sizeof(crop_buf), "w=iw-%u-%u:h=ih-%u-%u:x=%u:y=%u",
             ifp->opts.crop_left, ifp->opts.crop_right, ifp->opts.crop_top,
             ifp->opts.crop_bottom, ifp->opts.crop_left, ifp->opts.crop_top);
...
static int configure_input_filter(FilterGraph *fg, AVFilterGraph *graph,
  ...
  switch (ifilter->type) {
  case AVMEDIA_TYPE_VIDEO:
    return configure_input_video_filter(fg, graph, ifilter, in);
  case AVMEDIA_TYPE_AUDIO:
    return configure_input_audio_filter(fg, graph, ifilter, in);
  default:
    av_assert0(0);
    return 0;
  }
...
static int configure_filtergraph(FilterGraph *fg, FilterGraphThread *fgt) {
  ...
  if (simple) {
    ...
    if (av_dict_count(ofp->sws_opts)) {
      ret = av_dict_get_string(ofp->sws_opts, &fgt->graph->scale_sws_opts, '=',
                               ':');
      if (ret < 0)
        goto fail;
    }
  ...
  if (filter_buffered_frames) {
    ret = av_opt_set_int(fgt->graph, "max_buffered_frames",
  ...
  for (cur = inputs, i = 0; cur; cur = cur->next, i++)
    if ((ret = configure_input_filter(fg, fgt->graph, fg->inputs[i], cur)) <
        0) {
      avfilter_inout_free(&inputs);
      avfilter_inout_free(&outputs);
      goto fail;
...
static int ifilter_parameters_from_frame(InputFilter *ifilter,
  ...
  ifp->time_base = (ifilter->type == AVMEDIA_TYPE_AUDIO)
   ...
   * to the filter chain even though it's not "global", as filters like
   * aresample require this information during init and not when remixing a
   * frame */
  sd = av_frame_get_side_data(frame, AV_FRAME_DATA_DOWNMIX_INFO);
  if (sd) {
    ret = av_frame_side_data_clone(&ifp->side_data, &ifp->nb_side_data, sd, 0);
    if (ret < 0)
      return ret;
...
static void send_command(FilterGraph *fg, AVFilterGraph *graph, double time,
                         const char *target, const char *command,
                         const char *arg, int all_filters) {
  int ret;
  ...
  if (time < 0) {
    char response[4096];
    ret = avfilter_graph_send_command(graph, target, command, arg, response,
                                      sizeof(response),
                                      all_filters ? 0 : AVFILTER_CMD_FLAG_ONE);
    fprintf(stderr, "Command reply for stream %d: ret:%d res:\n%s", fg->index,
            ret, response);
  } else if (!all_filters) {
    fprintf(stderr, "Queuing commands only on filters supporting the specific "
...
static void video_sync_process(OutputFilterPriv *ofp, AVFrame *frame,
  ...
  sync_ipts = adjust_frame_pts_to_encoder_tb(ofilter->graph, frame, ofp->tb_out,
                                             ofp->ts_offset);
  ...
  *nb_frames = 1;
  ...
  if (delta0 < 0 && delta > 0 && fps->vsync_method != VSYNC_PASSTHROUGH
...
finish:
  ...
  if (*nb_frames >
      (*nb_frames_prev && fps->last_dropped) + (*nb_frames > *nb_frames_prev)) {
    ...
    nb_frames_dup =
        atomic_fetch_add(&ofilter->nb_frames_dup,
                         *nb_frames - (*nb_frames_prev && fps->last_dropped) -
  ...
  for (int64_t i = 0; i < nb_frames; i++) {
    ...
    if (type == AVMEDIA_TYPE_VIDEO) {
      ...
      frame->duration = av_rescale_q(
          frame->nb_samples, (AVRational){1, frame->sample_rate}, ofp->tb_out);
    ...
    ret =
        sch_filter_send(fgp->sch, fgp->sch_idx, ofp->ofilter.index, frame_out);
    if (ret < 0) {
...
static int send_eof(FilterGraphThread *fgt, InputFilter *ifilter, int64_t pts,
                    AVRational tb) {
  InputFilterPriv *ifp = ifp_from_ifilter(ifilter);
  int ret;
...
static int send_frame(FilterGraph *fg, FilterGraphThread *fgt,
  ...
  case AVMEDIA_TYPE_AUDIO:
    ...
    break;
  case AVMEDIA_TYPE_VIDEO:
    if (ifp->format != frame->format || ifp->width != frame->width ||
        ifp->height != frame->height || ifp->color_space != frame->colorspace ||
        ifp->color_range != frame->color_range)
  ...
  if (need_reinit && fgt->graph &&
      (ifp->opts.flags & IFILTER_FLAG_DROPCHANGED)) {
    ifp->nb_dropped++;
  ...
  if (need_reinit || !fgt->graph) {
    ...
    if (fgt->graph) {
      ...
      if (need_reinit & AUDIO_CHANGED) {
        ...
        av_bprintf(&reason, "audio parameters changed to %d Hz, ",
                   frame->sample_rate);
...
static int fg_thread_init(FilterGraphThread *fgt, const FilterGraph *fg) {
  ...
  fgt->frame_queue_out =
      av_fifo_alloc2(1, sizeof(AVFrame *), AV_FIFO_FLAG_AUTO_GROW);
  if (!fgt->frame_queue_out)
    goto fail;
...
static int filter_thread(void *arg) {
  ...
  while (1) {
    ...
    input_status =
        sch_filter_receive(fgp->sch, fgp->sch_idx, &input_idx, fgt.frame);
    ...
    if (ifp->type_src == AVMEDIA_TYPE_SUBTITLE) {
      ...
      ret = sub2video_frame(ifilter,
                            (fgt.frame->buf[0] || hb_frame) ? fgt.frame : NULL,
                            !fgt.graph);
    ...
    if (ret == AVERROR_EOF) {
      ...
      if (ifp && ifp->opts.flags & IFILTER_FLAG_DROPCHANGED)
        av_log(fg, AV_LOG_INFO,
               "Total changed input frames dropped : %" PRId64 "\n",
               ifp->nb_dropped);
      break;
    } else if (ret < 0) {
...
finish:
  ...
  // Graph printing removed - not needed
...
void fg_send_command(FilterGraph *fg, double time, const char *target,
  ...
  buf = av_buffer_create((uint8_t *)fc, sizeof(*fc), filter_command_free, NULL,
                         0);
  if (!buf) {
    av_freep(&fc);
    return;
