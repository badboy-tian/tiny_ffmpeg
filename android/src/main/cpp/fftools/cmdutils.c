...
void show_help_children(const AVClass *av_class, int flags) {
  ...
  const AVClass *child;
  if (av_class->option) {
    av_opt_show2(&av_class, NULL, flags, 0);
    printf("\n");
  }
...
static int write_option(void *optctx, const OptionDef *po, const char *opt,
   ...
   * a global var*/
  void *dst = po->flags & OPT_FLAG_OFFSET ? (uint8_t *)optctx + po->u.off
                                          : po->u.dst_ptr;
  ...
  if (po->flags & OPT_FLAG_SPEC) {
    ...
    if (po->flags & OPT_FLAG_PERSTREAM) {
      ret = stream_specifier_parse(&sol->opt[sol->nb_opt - 1].stream_spec, str,
                                   0, NULL);
      if (ret < 0)
        goto finish;
    }
  ...
  if (po->type == OPT_TYPE_STRING) {
    ...
    *(int *)dst = num;
    ...
    ret = parse_number(opt, arg, OPT_TYPE_INT64, INT64_MIN, (double)INT64_MAX,
                       &num);
    if (ret < 0)
      goto finish;
  ...
  if (sol) {
    sol->type = so_type;
    sol->opt_canon =
        (po->flags & OPT_HAS_CANON) ? find_option(defs, po->u1.name_canon) : po;
  }
...
int stream_specifier_parse(StreamSpecifier *ss, const char *spec,
  ...
  while (*spec) {
    if (*spec <= '9' && *spec >= '0') { /* opt:index */
      ...
      switch (*spec++) {
      case 'v':
        ss->media_type = AVMEDIA_TYPE_VIDEO;
        break;
      case 'a':
        ss->media_type = AVMEDIA_TYPE_AUDIO;
        break;
      case 's':
        ss->media_type = AVMEDIA_TYPE_SUBTITLE;
        break;
      case 'd':
        ss->media_type = AVMEDIA_TYPE_DATA;
        break;
      case 't':
        ss->media_type = AVMEDIA_TYPE_ATTACHMENT;
        break;
      case 'V':
        ss->media_type = AVMEDIA_TYPE_VIDEO;
        ss->no_apic = 1;
      ...
      for (len = 0;
           cmdutils_isalnum(spec[len]) || spec[len] == '_' || spec[len] == '+';
           len++)
        continue;
      ...
      av_log(logctx, AV_LOG_TRACE, "Parsed metadata: %s:%s; remainder: %s",
             ss->meta_key, ss->meta_val ? ss->meta_val : "<any value>", spec);
...
unsigned stream_specifier_match(const StreamSpecifier *ss,
  ...
  case STREAM_LIST_GROUP_IDX:
    if (ss->stream_list == STREAM_LIST_GROUP_IDX && ss->list_id >= 0 &&
        ss->list_id < s->nb_stream_groups)
  ...
  for (int i = start_stream; i < nb_streams; i++) {
    ...
    if (ss->meta_key) {
      const AVDictionaryEntry *tag =
          av_dict_get(candidate->metadata, ss->meta_key, NULL, 0);
