...
static DecoderPriv *dp_from_dec(Decoder *d) { return (DecoderPriv *)d; }
...
static AVRational audio_samplerate_update(DecoderPriv *dp,
  ...
  if (dp->last_frame_pts != AV_NOPTS_VALUE)
    dp->last_frame_pts =
        av_rescale_q(dp->last_frame_pts, dp->last_frame_tb, tb_new);
...
static void audio_ts_process(DecoderPriv *dp, AVFrame *frame) {
  ...
  pts_pred = dp->last_frame_pts == AV_NOPTS_VALUE
                 ? 0
  ...
  dp->last_frame_duration_est = av_rescale_q(frame->nb_samples, tb_filter, tb);
...
static int64_t video_duration_estimate(const DecoderPriv *dp,
  ...
  const int duration_unreliable =
      frame->duration == 1 && ts_diff > 2 * frame->duration;
  ...
  if (dp->dec_ctx->framerate.den && dp->dec_ctx->framerate.num) {
    ...
    AVRational field_rate =
        av_mul_q(dp->dec_ctx->framerate, (AVRational){2, 1});
    codec_duration =
  ...
  if (dp->framerate_in.num && dp->framerate_in.den) {
    int64_t d = av_rescale_q(1, av_inv_q(dp->framerate_in), frame->time_base);
    if (d > 0)
      return d;
  }
...
static int video_frame_process(DecoderPriv *dp, AVFrame *frame,
  ...
  if (frame->pts == AV_NOPTS_VALUE)
    frame->pts = dp->last_frame_pts == AV_NOPTS_VALUE
                     ? 0
                     : dp->last_frame_pts + dp->last_frame_duration_est;
...
static int process_subtitle(DecoderPriv *dp, AVFrame *frame) {
  ...
  if (dp->flags & DECODER_FLAG_FIX_SUB_DURATION) {
    AVSubtitle *sub_prev = dp->sub_prev[0]->buf[0]
                               ? (AVSubtitle *)dp->sub_prev[0]->buf[0]->data
                               : NULL;
    int end = 1;
    if (sub_prev) {
...
static int transcode_subtitles(DecoderPriv *dp, const AVPacket *pkt,
  ...
  if (pkt && (intptr_t)pkt->opaque == PKT_OPAQUE_SUB_HEARTBEAT) {
    ...
    return fix_sub_duration_heartbeat(
        dp, av_rescale_q(pkt->pts, pkt->time_base, AV_TIME_BASE_Q));
  }
...
static int packet_decode(DecoderPriv *dp, AVPacket *pkt, AVFrame *frame) {
  ...
  while (1) {
    ...
    if (dec->codec_type == AVMEDIA_TYPE_AUDIO) {
      ...
      if (ret < 0) {
        av_log(dp, AV_LOG_FATAL, "Error while processing the decoded data\n");
        return ret;
      }
    }
...
static int decoder_thread(void *arg) {
  ...
  while (!input_status) {
    ...
    if (!dp->dec_ctx) {
      if (flush_buffers)
        continue;
      if (input_status < 0) {
        av_log(dp, AV_LOG_ERROR, "Cannot initialize a standalone decoder\n");
        ret = input_status;
        goto finish;
      }
  ...
  if (ret >= 0) {
    ...
    dt.frame->pts = dp->last_frame_pts == AV_NOPTS_VALUE
                        ? AV_NOPTS_VALUE
                        : dp->last_frame_pts + dp->last_frame_duration_est;
    ...
    if (ret < 0 && ret != AVERROR_EOF) {
      av_log(dp, AV_LOG_FATAL, "Error signalling EOF timestamp: %s\n",
             av_err2str(ret));
      goto finish;
    }
    ret = 0;
...
static int multiview_setup(DecoderPriv *dp, AVCodecContext *dec_ctx) {
  ...
  if (nb_view_ids_av) {
    ...
    ret =
        av_opt_get_array(dec_ctx, "view_ids_available", AV_OPT_SEARCH_CHILDREN,
                         0, nb_view_ids_av, AV_OPT_TYPE_UINT, view_ids_av);
    if (ret < 0)
      goto fail;
  ...
  ret = av_opt_set_array(dec_ctx, "view_ids", AV_OPT_SEARCH_CHILDREN, 0,
                         nb_view_ids, AV_OPT_TYPE_INT, view_ids);
  if (ret < 0)
    goto fail;
...
static int hw_device_setup_for_decode(DecoderPriv *dp, const AVCodec *codec,
    ...
    for (int i = 0; !dev; i++) {
      ...
      if (hwaccel_device) {
        av_log(dp, AV_LOG_INFO,
               ...
               "from %s.\n",
               av_hwdevice_get_type_name(type), hwaccel_device);
      } else {
        av_log(dp, AV_LOG_INFO,
               "Using auto "
...
static int dec_open(DecoderPriv *dp, AVDictionary **dec_opts,
  ...
  if (ret < 0) {
    av_log(dp, AV_LOG_ERROR, "Hardware device setup failed for decoder: %s\n",
           av_err2str(ret));
    return ret;
  }
  ...
  if (param_out) {
    if (dp->dec_ctx->codec_type == AVMEDIA_TYPE_AUDIO) {
      ...
      ret = av_channel_layout_copy(&param_out->ch_layout,
                                   &dp->dec_ctx->ch_layout);
      if (ret < 0)
        return ret;
    ...
    ret = clone_side_data(&param_out->side_data, &param_out->nb_side_data,
                          dp->dec_ctx->decoded_side_data,
                          dp->dec_ctx->nb_decoded_side_data, 0);
    if (ret < 0)
      return ret;
...
int dec_init(Decoder **pdec, Scheduler *sch, AVDictionary **dec_opts,
             const DecoderOpts *o, AVFrame *param_out) {
  DecoderPriv *dp;
  int ret;
